...
export default function EditSavingsGoalForm({ editGoal, updateGoal }) {
    const [goal, setGoal] = useState(editGoal.goal);
    const [amount, setAmount] = useState(editGoal.amount);
    const [progress, setProgress] = useState(editGoal.progress);
    const [targetDate, setTargetDate] = useState(editGoal.targetDate);

    useEffect(() => {
        setGoal(editGoal.goal);
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        const updatedGoal = { ...editGoal, goal, amount: parseFloat(amount), progress: parseFloat(progress), targetDate };
    ...
    return (
        <div className="block w-fit p-6 bg-white border border-gray-200 rounded-lg shadow hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">
            ...
            <h5 className="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">Edit Goal</h5>
                ...
                <form className="space-y-3" onSubmit={handleSubmit}>
                    <div className="flex-1 rounded-lg bg-gray-50 px-6 pb-4 pt-8">
                        <div className="w-full">
                            ...
                            {/* Progress Input */}
                            <div>
                                <label className="mb-3 mt-5 block text-xs font-medium text-gray-900" htmlFor="password">Progress</label>
                                <div className="relative">
                                    <input className="peer block w-full rounded-md border border-gray-200 py-[9px] pl-10 text-sm outline-2 placeholder:text-gray-500 focus:border-blue-500 focus:border-2 focus-visible:outline-none" type="number" value={progress} onChange={(e) => setProgress(e.target.value)} placeholder="Saved Amount" required />
                                    <CurrencyDollarIcon className="pointer-events-none absolute left-3 top-1/2 h-[18px] w-[18px] -translate-y-1/2 text-gray-500 peer-focus:text-blue-500" />
                                </div>
                            </div>
                            {/* Date Input */}
